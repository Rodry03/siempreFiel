...
28. **Fórmula saldo turnos:** `sum(valores_semana) - 1` por semana (no `sum(valores_semana)`). 1 turno = neutro (0), no +1. Aplica igual en Python (`calcular_saldo`) y dbt (`mart_saldo_turnos_semanal`). La semana actual nunca penaliza. Excepción: semanas de verano (15/06–15/09) con 0 turnos tampoco penalizan (ver decisión 54).
...
54. **Saldo único + reset a verano 2026 + regla de verano:** se eliminó el saldo efectivo manual (decisión 39) para quedarse con un único saldo calculado. `FECHA_INICIO_SALDO`/`fecha_inicio_turnos` pasó de `2025-08-04` a `2026-06-15` — no se arrastra ninguna deuda ni saldo positivo anterior, todo el mundo empieza en 0 esa fecha (o en su `fecha_veterano` si es posterior). Además, las semanas dentro de la ventana de verano (15/06–15/09, todos los años) con 0 turnos no penalizan (quedan en 0 en vez de −1); si hubo algún turno esa semana (incluido medio_turno) se aplica la fórmula normal sin excepción. Una semana se considera "de verano" según la fecha de su lunes (no por mayoría de días). Implementado en `calcular_saldo()`/`detalle_voluntario` (`turnos.py`) y en `mart_saldo_turnos_semanal` (dbt); los marts legacy no usados en la app (`mart_saldo_turnos`, `mart_faltas_voluntario`) ya no recalculan el saldo con su propia fórmula — lo toman de `mart_saldo_turnos_semanal.saldo_acumulado` para no divergir.
