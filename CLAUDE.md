...
- **Saldo único, sin saldo manual/efectivo** (eliminado `Voluntario.saldo_manual`/`notas_saldo_manual`): solo existe el saldo calculado, no hay anotación manual paralela del gestor.
- **Saldo in-app** (`app/routers/turnos.py: calcular_saldo`):
  - Fecha inicio: `max(2026-06-15, fecha_veterano or fecha_alta)` (constante `FECHA_INICIO_SALDO`). Antes de esa fecha no se cuenta nada — reset total, sin arrastrar deudas ni saldos positivos previos.
  ...
  - **Regla de verano** (`_es_semana_verano`, 15/06–15/09 de cada año, según el lunes de la semana): si una semana de verano tiene 0 turnos, no penaliza (queda en 0 en vez de −1). Si hubo algún turno (incluido medio_turno), se aplica la fórmula normal sin excepción — la exención solo cubre 0 turnos.
  - Semana actual (incompleta) no se procesa.
  - **Saldo = suma de contribuciones semanales**
- **Saldo dbt** (`mart_saldo_turnos_semanal`): misma fecha de inicio (var `fecha_inicio_turnos` en `dbt_project.yml`), misma fórmula y misma regla de verano que el cálculo in-app. `mart_saldo_turnos` y `mart_faltas_voluntario` (marts legacy no usados en la UI) ya no calculan su propio saldo con una fórmula distinta — su columna `saldo` sale de `saldo_acumulado` de `mart_saldo_turnos_semanal` para no divergir.
- **Historial en perfil:** muestra todas las semanas desde 15/06/2026 (o `fecha_alta`/`fecha_veterano` si es posterior). Columna "Saldo" muestra contribución semanal (+1, 0, −1, ±0.5). Semanas sin turno fuera de verano → fila roja, −1. Semanas sin turno dentro de la ventana de verano → fila azul clara "Sin turno (verano, no penaliza)", 0. Semanas en PeriodoApoyo → badge azul, sin penalización. Semana actual → badge "Esta semana", sin −1.
- **Saldo en lista de voluntarios:** columna única "Saldo" con badge verde/rojo/gris calculado con `calcular_saldo`.
...
app/
  ...
  templates/
    ...
    voluntarios/
      list.html       — Active volunteers. Columna única: Saldo.
      detail.html     — Profile + historial turnos (desde 15/06/2026, semanas vacías en rojo con -1 salvo semanas de verano en azul claro sin penalizar, apoyo en azul). Tarjeta Saldo de turnos. Widget próximo evento con botón "Apuntarme" (solo veterano).
...
28. **Fórmula saldo turnos:** `sum(valores_semana) - 1` por semana (no `sum(valores_semana)`). 1 turno = neutro (0), no +1. Aplica igual en Python (`calcular_saldo`) y dbt (`mart_saldo_turnos_semanal`). La semana actual nunca penaliza. Excepción: en semanas de verano (15/06–15/09) no se resta 1 y los valores por turno cambian (ver decisión 54).
...
39. ~~Saldo efectivo paralelo al saldo automático~~ **(eliminado, ver decisión 54):** existió un `Voluntario.saldo_manual`/`notas_saldo_manual` anotado a mano por el gestor, independiente de `calcular_saldo()`. Se quitó por completo (código, UI y columnas en BD) al pasar a un único saldo calculado.
...
53. **Saltos de página explícitos en contrato_adopcion.docx:** incluso con la misma fuente (decisión 52), Word y LibreOffice no paginan de forma idéntica cuando la página se llena "de forma natural" — pequeñas diferencias de interlineado hacían que LibreOffice metiera un párrafo de más al final de cada página, descuadrando el documento frente a la versión de Word. Se fijó `paragraph_format.page_break_before = True` directamente en la plantilla (`app/contracts/contrato_adopcion.docx`) en los párrafos "3ª", "11ª", "19ª" y "25ª", así el documento siempre pagina igual (página 1: datos + tabla perro, página 2: 3ª–10ª, página 3: 11ª–18ª, página 4: 19ª–24ª, página 5: 25ª + firma) en cualquier motor de renderizado. Verificado generando el PDF con LibreOffice dentro de un contenedor con la imagen de Render y comparando visualmente las 5 páginas. Si aparecen saltos de página inconsistentes en otras plantillas (`contrato_acogida.docx`, `contrato_preadopcion.docx`), aplicar el mismo criterio en vez de confiar en el reflow automático.
54. **Saldo único + reset a verano 2026 + fórmula de verano:** se eliminó el saldo efectivo manual (decisión 39) para quedarse con un único saldo calculado. `FECHA_INICIO_SALDO`/`fecha_inicio_turnos` pasó de `2025-08-04` a `2026-06-15` — no se arrastra ninguna deuda ni saldo positivo anterior, todo el mundo empieza en 0 esa fecha (o en su `fecha_veterano` si es posterior). Dentro de la ventana de verano (15/06–15/09, todos los años) la fórmula semanal cambia y **no se resta 1**: un turno completo (`realizado`) suma **0.5**, el medio turno **no cuenta** (0). Ejemplos en verano: 0 turnos=0, 1 turno=+0.5, 2 turnos=+1, medio_turno=0. Fuera de verano se mantiene la fórmula de siempre (`valor - 1` con realizado=1.0/medio_turno=0.5, decisión 28). Una semana se considera "de verano" según la fecha de su lunes (no por mayoría de días). Implementado en `calcular_saldo()`/`detalle_voluntario` (`turnos.py`) y en `mart_saldo_turnos_semanal` (dbt, columna `valor_verano` en `turnos_por_semana`); los marts legacy no usados en la app (`mart_saldo_turnos`, `mart_faltas_voluntario`) ya no recalculan el saldo con su propia fórmula — lo toman de `mart_saldo_turnos_semanal.saldo_acumulado` para no divergir.
